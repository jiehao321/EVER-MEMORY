...
function buildMemoryInput(userId, question, answer) {
    const base = {
        scope: { userId },
...
function appendOnboardingHint(profile) {
...
export class OnboardingService {
    ...
    constructor(memoryService, memoryRepo, profileRepo, smartnessMetricsService) {
        this.memoryService = memoryService;
        this.memoryRepo = memoryRepo;
        this.profileRepo = profileRepo;
        this.smartnessMetricsService = smartnessMetricsService;
    }
    isOnboardingNeeded(userId) {
    ...
    generateWelcomeMessage(isFirstRun, userName, userId) {
        if (isFirstRun) {
            return [
    ...
    async processResponses(userId, responses) {
        const normalizedUserId = userId.trim();
        if (!normalizedUserId) {
            return {
                completed: false,
        ...
        for (const response of responses) {
            ...
            const clippedAnswer = answer.length > MAX_ONBOARDING_ANSWER_LENGTH
                ...
                : answer;
            const stored = this.memoryService.store(buildMemoryInput(normalizedUserId, question, clippedAnswer));
            if (stored.accepted) {
                memoriesCreated += 1;
            }
        }
    ...
    resolveSmartnessScore(memoryCount) {
