import { COMMITMENT_PATTERNS, IDENTITY_PATTERNS, LOW_VALUE_PATTERNS, WRITE_CONSTRAINT_PATTERNS as CONSTRAINT_PATTERNS, WRITE_DECISION_PATTERNS as DECISION_PATTERNS, WRITE_PREFERENCE_PATTERNS as PREFERENCE_PATTERNS, } from '../../patterns.js';
import { WRITE_CONFIDENCE_BY_TYPE, WRITE_DEFAULT_EXPLICITNESS, WRITE_IMPORTANCE_BY_TYPE, WRITE_MAX_CONTENT_LENGTH, WRITE_MIN_CONTENT_LENGTH, } from '../../tuning.js';
...
function inferType(content, fallback) {
    if (fallback) {
        return fallback;
    }
...
function inferLifecycle(type, fallback) {
    if (fallback) {
        return fallback;
    }
...
function inferExplicitness(content) {
    if (/\bremember\b/i.test(content) || /记住|记一下/.test(content)) {
        return 1;
    }
    return WRITE_DEFAULT_EXPLICITNESS;
}
function resolveScore(value, fallback, field, input) {
    if (value === undefined) {
        return fallback;
    }
...
function addStrippedPatterns(decision, strippedPatterns, cleanedContent) {
