...
function normalizeType(value) {
    return typeof value === 'string' && MEMORY_TYPES.includes(value) ? value : null;
}
function fallbackLifecycle(type) {
...
function toMemoryItem(entry, scope) {
...
export class MemoryExportService {
    ...
    async import(content, format, scope) {
        const errors = [];
        let imported = 0;
        let skipped = 0;
        let rawItems = [];
        try {
            rawItems = format === 'json' ? JSON.parse(content) : parseMarkdown(content);
        ...
        const existing = new Set(this.memoryRepo.search({
            scope,
            archived: undefined,
