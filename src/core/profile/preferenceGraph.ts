...
export interface PreferenceNode {
  readonly id: string;
  readonly category: 'tech' | 'style' | 'principle' | 'workflow' | 'domain';
...
export interface PreferenceEdge {
  ...
  readonly relation: 'implies' | 'conflicts' | 'reinforces' | 'domain_of';
  readonly weight: number;
}

export interface PreferenceGraph {
  readonly nodes: readonly PreferenceNode[];
...
function freezeNode(node: PreferenceNode): PreferenceNode {
  return Object.freeze(node);
}

function freezeEdge(edge: PreferenceEdge): PreferenceEdge {
  return Object.freeze(edge);
}

function inferCategory(key: string, label: string): PreferenceNode['category'] {
...
function collectBaseNodes(profile: ProjectedProfile): PreferenceNode[] {
...
export class PreferenceGraphService {
  buildFromProfile(userId: string, profile: ProjectedProfile): PreferenceGraph {
    const nodeMap = new Map<string, PreferenceNode>();
  ...
  inferImplications(graph: PreferenceGraph): readonly string[] {
  ...
  findConflicts(graph: PreferenceGraph): readonly { nodeA: string; nodeB: string; reason: string }[] {
  ...
  getTopPreferences(graph: PreferenceGraph, limit = 5): readonly PreferenceNode[] {
    return Object.freeze(
      graph.nodes
