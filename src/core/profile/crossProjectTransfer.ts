...
export interface TransferablePreference {
...
function resolveKind(memory: MemoryItem): string | null {
...
function score(memory: MemoryItem): number {
  return memory.scores.importance * memory.scores.confidence;
}

function toTransferablePreference(memory: MemoryItem): TransferablePreference | null {
...
export class CrossProjectTransferService {
  ...
  getGlobalPreferences(userId: string): readonly TransferablePreference[] {
    return this.memoryRepo.search({
      ...
      .filter((entry): entry is { memory: MemoryItem; preference: TransferablePreference } => Boolean(entry.preference))
      .sort((left, right) => score(right.memory) - score(left.memory))
      .slice(0, 20)
      .map((entry) => entry.preference);
  }

  getTransferableTo(userId: string, targetProject: string): readonly TransferablePreference[] {
  ...
  shouldInheritGlobal(preference: TransferablePreference): boolean {
