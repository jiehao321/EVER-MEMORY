...
function makeRule(input: Partial<BehaviorRule> & Pick<BehaviorRule, 'statement' | 'category' | 'priority'>): BehaviorRule {
  ...
  return {
    ...
    lifecycle: {
      ...
      autoSuspended: input.lifecycle?.autoSuspended ?? false,
      autoSuspendedAt: input.lifecycle?.autoSuspendedAt,
