import { DATA_QUALITY_INFERENCE, DATA_QUALITY_RUNTIME, DATA_QUALITY_RUNTIME_LOW_VALUE, DATA_QUALITY_SUMMARY, DATA_QUALITY_TEST, DATA_QUALITY_UNKNOWN, DATA_QUALITY_UNKNOWN_LOW_VALUE, PROJECT_PRIORITY_COMMITMENT_NEXT_STEP, PROJECT_PRIORITY_CONSTRAINT, PROJECT_PRIORITY_DECISION, PROJECT_PRIORITY_DEFAULT, PROJECT_PRIORITY_PROJECT, PROJECT_PRIORITY_PROJECT_STATE, PROJECT_PRIORITY_SUMMARY, RETRIEVAL_DEFAULT_POLICY_WEIGHT_BASE, RETRIEVAL_DEFAULT_POLICY_WEIGHT_QUALITY, RETRIEVAL_PROJECT_POLICY_WEIGHT_BASE, RETRIEVAL_PROJECT_POLICY_WEIGHT_PROJECT, RETRIEVAL_PROJECT_POLICY_WEIGHT_QUALITY, } from '../../tuning.js';
...
export class RetrievalStrategySupport {
    ...
    constructor(memoryRepo, semanticCandidateLimit) {
        this.memoryRepo = memoryRepo;
        this.semanticCandidateLimit = semanticCandidateLimit;
    }
    applyRecallPolicyScore(memory, baseScore, meta) {
    ...
    applyCandidatePolicy(candidates, limit, meta) {
        const classCounts = {
            runtime: 0,
            test: 0,
            unknown: 0,
    ...
    loadCandidates(request, limit, queryEnabled, meta) {
    ...
    classifyMemoryData(memory) {
        const tags = memory.tags.map((tag) => tag.toLowerCase());
        const lowerContent = memory.content.toLowerCase();
        if (memory.source.kind === 'test'
