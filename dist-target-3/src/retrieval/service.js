import { DEFAULT_RETRIEVAL_HYBRID_WEIGHTS, DEFAULT_RETRIEVAL_KEYWORD_WEIGHTS, DEFAULT_SEMANTIC_SIDECAR_MAX_CANDIDATES, DEFAULT_SEMANTIC_SIDECAR_MIN_SCORE, } from '../constants.js';
...
import { resolveRecallLimit, resolvePositiveInteger, } from './strategies/support.js';
import { RetrievalError } from '../errors.js';
import { embeddingManager } from '../embedding/manager.js';
function resolveWeightNumber(value, fallback) {
...
function resolveKeywordWeights(weights) {
    return normalizeWeights({
        ...
        lifecyclePriority: resolveWeightNumber(weights?.lifecyclePriority, DEFAULT_RETRIEVAL_KEYWORD_WEIGHTS.lifecyclePriority),
    }, { ...DEFAULT_RETRIEVAL_KEYWORD_WEIGHTS });
}
function resolveHybridWeights(weights) {
    return normalizeWeights({
...
export class RetrievalService {
    ...
    constructor(memoryRepo, debugRepo, options = {}) {
        ...
        this.structuredStrategy = new StructuredRetrievalStrategy(support, this.maxRecall, this.keywordWeights, semanticEnabled, semanticRepo);
        this.keywordStrategy = new KeywordRetrievalStrategy(support, this.keywordWeights);
        this.hybridStrategy = new HybridRetrievalStrategy(support, semanticRepo, semanticEnabled, semanticCandidateLimit, semanticMinScore, this.keywordWeights, this.hybridWeights);
    }
    async recall(request, meta) {
        try {
            ...
            const degradedReason = result.degradationReason
                ?? (semanticFallback ? 'semantic_unavailable' : undefined);
            const degraded = Boolean(result.degradationReason);
            return {
                items,
                total: items.length,
                limit,
    ...
    async rankByMode(mode, request, limit, meta) {
        if (mode === 'structured') {
