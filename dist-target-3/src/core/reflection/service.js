...
import { REFLECTION_APPROVAL_BOOST, REFLECTION_CONFIDENCE_BASE, REFLECTION_CORRECTION_BOOST, REFLECTION_EVIDENCE_BOOST_MIN_REFS, REFLECTION_EVIDENCE_BOOST_VALUE, REFLECTION_FULL_CONFIDENCE_THRESHOLD, REFLECTION_FULL_EXPERIENCE_LIMIT, REFLECTION_LIGHT_CONFIDENCE_THRESHOLD, REFLECTION_LIGHT_EXPERIENCE_LIMIT, REFLECTION_MAX_RECURRENCE_BOOST, REFLECTION_MODE_PENALTY, REFLECTION_RECURRENCE_BOOST_INCREMENT, } from '../../tuning.js';
function nowIso() {
...
function summarizeAnalysis(triggerKind, experiences) {
...
function computeConfidence(triggerKind, experiences, evidenceRefs, mode) {
    const base = REFLECTION_CONFIDENCE_BASE[triggerKind];
    const correctionCount = experiences.filter((item) => item.indicators.userCorrection).length;
    const approvalCount = experiences.filter((item) => item.indicators.userApproval).length;
    const recurrenceBoost = Math.min(REFLECTION_MAX_RECURRENCE_BOOST, Math.max(0, experiences.length - 1) * REFLECTION_RECURRENCE_BOOST_INCREMENT);
...
export class ReflectionService {
    ...
    reflectionRepo;
    debugRepo;
    constructor(experienceRepo, reflectionRepo, debugRepo) {
    ...
    resolveExperiences(input, limit) {
        if (input.experienceIds && input.experienceIds.length > 0) {
            const experiences = [];
            const missingExperienceIds = [];
