...
import { KEYWORD_EMPTY_QUERY_SCORE, KEYWORD_PHRASE_IN_CONTENT_SCORE, KEYWORD_PHRASE_IN_TAGS_SCORE, KEYWORD_RECENCY_BRACKETS, KEYWORD_SCOPE_BASE_SCORE, KEYWORD_SCOPE_CHAT_BONUS, KEYWORD_SCOPE_GLOBAL_BONUS, KEYWORD_SCOPE_NO_REQUEST, KEYWORD_SCOPE_PROJECT_BONUS, KEYWORD_SCOPE_USER_BONUS, KEYWORD_TOKEN_COVERAGE_WEIGHT, } from '../../tuning.js';
...
function tokenizeQuery(normalizedQuery) {
    ...
    const uniqueTokens = Array.from(new Set(normalizedQuery
        .split(/[\s,.;:!?，。；：！？、/]+/g)
        .map((part) => part.trim())
...
function priorityByIndex(value, ordered) {
...
function computeScopeMatch(memory, request) {
    if (!request.scope) {
        return KEYWORD_SCOPE_NO_REQUEST;
    }
...
function keywordFactors(memory, normalizedQuery, queryTokens) {
    ...
    const matchedTokens = queryTokens.filter((token) => (normalizedContent.includes(token)
    ...
    const score = clamp01((phraseInContent ? KEYWORD_PHRASE_IN_CONTENT_SCORE : 0)
...
function scoreMemory(memory, request, normalizedQuery, queryTokens, weights) {
    ...
    const score = (factors.keyword * weights.keyword
...
function toStrategyItem(entry, support, meta) {
...
export function rankKeywordRecall(memories, request, options = {}) {
