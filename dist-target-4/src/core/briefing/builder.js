...
import { BRIEFING_CLIP_DEFAULT, BRIEFING_CLIP_PROJECT_SUMMARY, BRIEFING_CLIP_SHORT, BRIEFING_COMMITMENT_LIMIT, BRIEFING_MAX_ACTIVE_PROJECTS, BRIEFING_PICK_COMMITMENTS, BRIEFING_PICK_CONSTRAINTS, BRIEFING_PICK_CONTINUITY, BRIEFING_PICK_CONTINUITY_OUTPUT, BRIEFING_PICK_DECISIONS, BRIEFING_PICK_IDENTITY, } from '../../tuning.js';
...
export function clip(value, max = BRIEFING_CLIP_DEFAULT) {
...
function dedupeByContent(values) {
...
export function appendPreferenceSummary(identity, scope, graphService, profileRepo) {
...
function firstContent(memories, fallback = '') {
    if (memories.length === 0) {
        return fallback;
    }
    return clip(memories[0].content, BRIEFING_CLIP_SHORT);
}
function stripKnownLabel(content) {
    return content
...
function byProject(memories, projectName) {
    if (!projectName) {
...
function parseProjectSummary(content) {
...
function hasProjectStatusSignal(content) {
    return PROJECT_STATUS_PATTERNS.some((pattern) => pattern.test(content));
}
function pickProjectStatus(projectMemories, summaryStatus) {
...
function composeProjectSummary(input) {
    ...
    return clip(`项目连续性摘要（${input.projectName ?? 'current'}）：${parts.join('；')}`, BRIEFING_CLIP_PROJECT_SUMMARY);
}
export function composeActiveProjects(scope, projectSummaries, projectMemories, constraints, decisions, commitments) {
    const entries = [];
...
export function composeRecentContinuity(recentContinuity, decisions, commitments) {
    return dedupe([
...
export function resolveTokenTarget(tokenTarget) {
...
export function optimizeSections(sections, tokenTarget) {
    const order = ['activeProjects', 'constraints', 'recentContinuity', 'identity'];
    const deduped = {
        identity: [],
        constraints: [],
        recentContinuity: [],
    ...
    const pruneOrder = ['identity', 'recentContinuity', 'constraints', 'activeProjects'];
...
function toConciseProjectSummary(entry) {
...
export function adaptSectionsByStyle(sections, communicationStyle) {
    ...
    return {
        ...
        activeProjects: dedupeByContent(communicationStyle === 'concise'
            ? sections.activeProjects.map((entry) => toConciseProjectSummary(entry))
            : sections.activeProjects).slice(0, limit),
    };
}
