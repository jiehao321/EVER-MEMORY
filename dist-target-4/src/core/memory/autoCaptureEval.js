import { CONSTRAINT_PATTERNS, CORRECTION_PATTERNS, DECISION_PATTERNS, GENERIC_OUTCOME_PATTERNS, NEXT_STEP_PATTERNS, PREFERENCE_PATTERNS, PROJECT_COMPACT_PATTERNS, PROJECT_PATTERNS, PROJECT_STATUS_PATTERNS, TEST_NOISE_PATTERNS, } from '../../patterns.js';
import { AUTO_CAPTURE_CLIP_DEFAULT, AUTO_CAPTURE_CLIP_INPUT, AUTO_CAPTURE_CLIP_OUTCOME, AUTO_CAPTURE_CORRECTION_SIGNAL_THRESHOLD, AUTO_CAPTURE_MIN_CONTENT_LENGTH, AUTO_CAPTURE_MIN_QUALITY, AUTO_CAPTURE_PREFERENCE_RELEVANCE_THRESHOLD, AUTO_CAPTURE_SUMMARY_MIN_SIGNALS, } from '../../tuning.js';
function clip(value, max = AUTO_CAPTURE_CLIP_DEFAULT) {
    if (!value) {
        return '';
    }
...
function shouldSkipAutoCapture(text) {
...
function stripExperiencePlaceholder(value) {
    const normalized = (value ?? '').trim();
...
function deriveNextStep(input, reflection, sourceText) {
...
function hasProjectStatusSignal(value) {
    return containsAny(value, PROJECT_STATUS_PATTERNS);
}
function extractSummaryField(content, label) {
...
export function evaluateCandidateQuality(candidate) {
...
export function buildAutoMemoryCandidates(input, context) {
    ...
    const actionSummary = clip(normalizeAssistantOutput(input.actionSummary || stripExperiencePlaceholder(experience.actionSummary)), AUTO_CAPTURE_CLIP_INPUT);
    const outcomeSummary = clip(input.outcomeSummary || stripExperiencePlaceholder(experience.outcomeSummary), AUTO_CAPTURE_CLIP_OUTCOME);
    ...
    const sourceRuntimeProject = {
        ...sourceBase,
        kind: 'runtime_project',
    };
    const sourceRuntimeUser = {
        ...sourceBase,
        kind: 'runtime_user',
    };
    const sourceReflectionDerived = {
        ...sourceBase,
    ...
    const keyConstraint = clip(reflection?.analysis.nextTimeRecommendation
        || (constraintHint ? '先复述修正点并确认，再继续执行。' : ''), AUTO_CAPTURE_CLIP_OUTCOME);
